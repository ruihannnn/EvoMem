...
def llm(prompt,stop=None, model=None):
    ...
    # Use the same LLM configuration as the main project
    api_key = os.environ.get("UTU_LLM_API_KEY") or os.environ.get("OPENAI_API_KEY", "YOUR_API_KEY")
    base_url = os.environ.get("UTU_LLM_BASE_URL") or os.environ.get("OPENAI_API_BASE", "YOUR_API_BASE")
    
    # If model is not provided, try to get it from environment variables
    if model is None:
        model = os.environ.get("UTU_LLM_MODEL") or os.environ.get("JUDGE_LLM_MODEL") or "gpt-4.1"
...
def adjust_memory(worfklow, reward, trajectory):
    ...
    # Use the model from environment variables instead of default
    model = os.environ.get("UTU_LLM_MODEL") or os.environ.get("JUDGE_LLM_MODEL") or "gpt-4.1"
    output = llm(input_prompt, model=model)
